...
This `.rules` file defines **hard** (must-follow) and **soft** (strongly recommended) rules for working on this Python library. The intent is to maximize code quality, safety, maintainability, and effective use of coding assistants in the context of developing `mcp-refcache` - a reference-based caching library for FastMCP servers.
...
- Keep runtime dependencies minimal - users shouldn't inherit a bloated dependency tree
- Make optional features truly optional with extras (e.g., `mcp-refcache[redis]`)
...
## Python Library Design Guidelines

### Hard Rules
- **Stable Public API**: All public classes, functions, and types must be exported from `__init__.py`
- **Semantic Versioning**: Follow semver strictly. Breaking changes require major version bump (after 1.0.0)
- **Type Stubs**: Include `py.typed` marker file for PEP 561 compliance
- **No Side Effects on Import**: Importing the library should not execute code, create files, or make network calls
- **Explicit Over Implicit**: Users should understand what they're getting without reading source code

### Soft Rules
- **Minimal Dependencies**: Every dependency is a liability. Prefer stdlib when reasonable.
- **Optional Dependencies**: Use extras for non-core features (e.g., `mcp-refcache[redis]`, `mcp-refcache[tiktoken]`)
- **Sensible Defaults**: Library should work out-of-the-box with zero configuration
- **Escape Hatches**: Provide ways for advanced users to customize behavior
- **Progressive Disclosure**: Simple use cases should be simple; complex features can require more setup
- **Backwards Compatibility**: Deprecate before removing. Provide migration paths.

### API Design Principles
- **Consistency**: Similar operations should have similar signatures
- **Predictability**: No surprising behavior or hidden state
- **Composability**: Components should work well together and with external code
- **Error Messages**: Exceptions should tell users what went wrong AND how to fix it
...
### Hard Rules
- **All tests must pass before publishing**: `pytest` with 100% pass rate
- **Linting must pass**: `ruff check .` with no errors
- **Version must be bumped**: Never publish the same version twice
- **CHANGELOG must be updated**: Document what changed in this version

### Soft Rules
- Test installation in a clean virtualenv before publishing
- Build and inspect the wheel: `uv build && unzip -l dist/*.whl`
- Consider publishing to TestPyPI first for major releases
